...
| 3.5.0 | 3.1.0 | Same wire contract. SDK can now populate the existing envelope `location` field at runtime via opt-in IP geolocation (`resolveLocation` + `locationProviderUrl` config). Backend behavior unchanged — top-level `location` has been accepted since 3.0.0. |
