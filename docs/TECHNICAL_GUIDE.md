...
| `captureFrames` | `true` | Emits `frame_render_time` metrics (web: one windowed summary per screen; iOS CADisplayLink + Android Choreographer per-sample) |
| `captureAllFrames` | `false` | No-op on web since ADR-030 (windowed aggregation); still honoured by native samplers |
| `frameSlowThresholdMs` | `16.67` | Classifies a slow frame; drives web `slow_frames`/`dropped_frames` (≥ 2× = dropped) |
